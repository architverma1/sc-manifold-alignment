...
parser.add_argument('--batch_var', dest = 'batch_var', type = bool, default = False, help = 'include binary fish/not fish')

parser.add_argument('--sparse', dest = 'sparse', type = bool, default = False, help = 'Data saved in sparse format (CSC/CSR)')
parser.add_argument('--pca_init', dest = 'pca_init', type = bool, default = False, help = 'Initialize with PCA/TruncatedSVD (True) or Randomly (False)')
args = parser.parse_args()


INDICATOR = args.batch_var
SIMULATED = False
...
# load data
dat_path = '/home/archithpc/data/cortex_svz_combined.hdf5'
#filtered = mmread(dat_path)
#y_train = csc_matrix(filtered).T
#z_read = pd.read_csv('/home/archithpc/data/pancreas-combined/ident.csv')
#z_init = pd.get_dummies(z_read['x']).values[:,0:3]
dat = h5py.File(dat_path, 'r')
# 0:523 --> cortex, 524:803 --> SVZ, 803:end --> Choroid Plexus
y_train = np.vstack((dat['fish_shared'][0:523,:],np.log2(1. + dat['rna_shared'][:])))
z_init = .01*dat['coords'][0:523]

Nfixed = dat['fish_shared'][0:523].shape[0]
Nvar = dat['rna_shared'].shape[0]

from scipy.spatial.distance import pdist, squareform

#c = pdist(y_train,metric='cosine')
#cmat = squareform(c)
#cross_d = cmat[Nfixed:,0:Nfixed]
#cross_cell = np.argmax(cross_d, axis = 1)
#zvar_init = z_init[cross_cell]

zbatch_init = np.vstack((np.zeros((Nfixed,1)),np.zeros((Nvar,1))))
#print(zbatch_init.shape)
zvar_init = np.random.normal(np.mean(z_init, axis = 0), np.sqrt(np.var(z_init, axis = 0)), size = (Nvar, 2))

#kernel = GPy.kern.RBF(input_dim=z_init.shape[1])
#model = GPy.models.GPRegression(z_init,y_train,kernel)
#model.optimize(messages=True)
#corrected_zero = model.predict(z_init)
#y_init = y_train - corrected_zero[0]
...
len_con_init = 0.1*np.abs(np.max(z_init, axis = 0) - np.mean(z_init,axis = 0))
len_init = np.concatenate((len_var_init, len_con_init))
if INDICATOR:
	len_init = np.concatenate((len_init, np.array([0.5])))
print(len_init.shape)

# Initialize X_u with random subset of initalization
xu_init = np.random.choice(Nfixed, size = m)
xu_var = tf.Variable(qx_init[xu_init], dtype = tf.float64)
qu_init = y_train[xu_init] 

xu_con = tf.Variable(z_init[xu_init], dtype = tf.float64)
xu1 = tf.concat([xu_var,xu_con], 1)
print('xu1')
print(xu1.shape)

if INDICATOR:
	xu_batch = tf.Variable(zbatch_init[xu_init], dtype = tf.float64)
	xu = tf.concat([xu1, xu_batch], 1)
else:
	xu = xu1
print('xu')
print(xu.shape)
...
zfixed = tf.constant(z_init, dtype = tf.float32)
print('zfixed')
print(zfixed.shape)

zvar = tf.Variable(zvar_init, dtype = tf.float32)
print('zvar')
print(zvar.shape)


z1 = tf.concat([zfixed, zvar], 0, name = 'z')
print('z1')
print(z1.shape)
if INDICATOR:
	zbatch_fixed = tf.constant(zbatch_init, dtype = tf.float32) # added binary
	z = tf.concat([z1, zbatch_fixed], 1, name = 'z_with_batch')
else:
	z = z1
print('z')
print(z.shape)
...
qx_var = Normal(loc = qx_mini, scale = tf.nn.softplus(qx_scale_mini), name = 'qx')



qz_fixed = tf.constant(z_init, dtype = tf.float32, name = 'qz_fixed')
qz1 = tf.concat([qz_fixed, zvar], 0, name = 'qz')
print('qz1')
print(qz1.shape)

if INDICATOR:
	qz_batch = tf.gather(zbatch_fixed, idx_ph) # binary indicator var
	qz = tf.concat([qz1, qz_batch], 1, name = 'qz_with_batch') # binary indicator var
else:
	qz = qz1
print('qz')
print(qz.shape)
qx = tf.concat([qx_var, tf.gather(qz,idx_ph)], 1, name = 'qx_full')
...
def save(name):
    ...
    fout.create_dataset("qx_init", data = qx_init)
    fout.create_dataset("z_post", data = qz1.eval())
    fout.create_dataset("zvar_init", data = zvar_init)
    if SIMULATED:
...
## Run Inference
inference = ed.KLqp({f: qf, x_var: qx_var, u: qu}, data = {y: y_dat_ph})
