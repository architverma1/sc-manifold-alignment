...
parser.add_argument('--gpy_init', dest = 'gpy_init', type = bool, default = False, help = 'Initialize with GPLVM')
parser.add_argument('--lin_reg', dest = 'lin_reg', type = bool, default = False, help = 'Start with lin reg correction of batch')
parser.add_argument('--gpy_reg', dest = 'gpy_reg', type = bool, default = False, help = 'Start with gpy correction of batch')
...
#dat_path = '/home/archithpc/data/batch-effect-noise/' + str(args.k) + '/sim-cluster-' + str(args.batch) + '-' + str(args.j) + '.csv'
dat_path = '/home/archithpc/data/batch-effect-noise/sim-cluster-' + str(args.j) + '.csv'
#dat_path = '/home/archithpc/data/batch-variable-genes-2.npy'
y_load = np.loadtxt(dat_path)
#y_train = y_load[args.k,args.batch-1,args.j-1] 
y_train = y_load

z_init = np.concatenate((np.zeros((300,1)),np.ones((200,1))),axis = 0)
...
if args.gpy_reg:
	len_con_init = model.param_array[1] * np.ones(z_init.shape[1])
	sig_init = model.param_array[0]
...
variance_pre = tf.Variable(np.log(np.exp(sig_init)-1), dtype = tf.float32)
...
varianceM12_pre = tf.Variable(np.log(np.exp(sig_init)-1), dtype = tf.float32)
...
qz = tf.constant(z_init, dtype = tf.float32, name = 'qz')
