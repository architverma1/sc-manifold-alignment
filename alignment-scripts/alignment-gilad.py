...
# load data
dat_path = '/home/archithpc/data/gilad_processed.hdf5'
...
#z2 = dat['z_batch'][:]
#z3 = dat['z_c1'][:]
z4 = np.expand_dims(np.load('z4.npy'),axis = 1) # gender!
#z_init = np.hstack((z3,z4))
z_init = z4
...
if PCA_INIT:
	...
	#Yres = y_init
        if SPARSE:
		#x, istop, itn, normr = lsqr(csc_matrix(z_init), y_train)
		#predict = np.matmul(z_init, x)
		#Yres = y_train - predict
...
#variance_pre = tf.Variable(np.log(np.exp(model.param_array[0])-1), dtype = tf.float32)
