...
# load data
dat_path = '/home/archithpc/data/pancreas-mnn/raw_all.csv'
filtered = pd.read_csv(dat_path)
y_train = filtered.values[:,1:].T
#z_read = pd.read_csv('/home/archithpc/data/pancreas-combined/ident.csv')
z_init = np.load('/home/archithpc/data/pancreas-mnn/pancreas-mnn-z.npy')
...
if PCA_INIT:
	#y_train_temp = np.divide(y_train.T, np.sum(y_train, axis = 1)).T
...
len_con_init = .5*np.ones(z_init.shape[1])
